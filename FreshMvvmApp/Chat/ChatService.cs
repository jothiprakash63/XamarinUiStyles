...
namespace KantimeEvv.Chat
{
    public class ChatService : IChatService
    {
        public ChatService()
        {
            this.Init("192.168.43.252", false);
        }
        ...
        public void Init(string urlRoot, bool useHttps)
        {
            ...
            var url = "http://10.0.2.2:5229/hubs/chat";
        ...
        public async Task SendMessageToAll(string group, string userName, string message)
        {
            if (!IsConnected)
                throw new InvalidOperationException("Not connected");

            await hubConnection.InvokeAsync("SendMessageToAll",
                    group,
                    userName,
                    message);
        }

        public async Task SendPrivateMessage(string group, string userName, string message)
        {
            if (!IsConnected)
                throw new InvalidOperationException("Not connected");
